...
def rate_limited(chat_id: int) -> bool:
    """True if the chat exceeded RATE_LIMIT_PER_MINUTE in the last 60s."""
    now = time.monotonic()
...
def save_intruder_info(user_info: dict) -> None:
    """Atomic save with backup; never crashes the caller."""
    ...
    with _LOCK:
        try:
            ...
            try:
                with open(INTRUDERS_PATH, "r", encoding="utf-8") as f:
                    intruders = json.load(f)
            except (FileNotFoundError, json.JSONDecodeError):
                intruders = {}
                if INTRUDERS_PATH.exists():
                    try:
                        INTRUDERS_PATH.replace(
                            INTRUDERS_PATH.with_suffix(f".corrupted.{ts}")
                        )
                    except OSError:
                        pass
            ...
            fd, temp = tempfile.mkstemp(dir=str(INTRUDERS_PATH.parent))
            try:
                with os.fdopen(fd, "w", encoding="utf-8") as tmpf:
                    json.dump(intruders, tmpf, indent=4, ensure_ascii=False)
                    tmpf.flush()
                    os.fsync(tmpf.fileno())
                os.replace(temp, INTRUDERS_PATH)
            except Exception:
                if INTRUDERS_BAK.exists():
                    try:
            ...
            log.error("Failed to save intruder: %s\n%s", e, traceback.format_exc())
